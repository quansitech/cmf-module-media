...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | 字段声明 cropAspectRatio() 后，图片在浏览器里裁好再上传（裁剪先于内容
    | 指纹计算，故秒传去重基于裁剪后的文件）。
    |
    | max_width：裁剪产物的最大像素宽，超出等比缩小；手机原图直传意义不大，
    |            反而拖慢上传，默认限制到 1600。
    | quality：  JPEG 重编码质量。PNG 源保持 PNG 以留住透明通道，该值不生效。
    |
    | 比例只是前端约束，服务端不做校验。
    |
    */
    ...
    'crop' => [
        'max_width' => env('CMF_MEDIA_CROP_MAX_WIDTH', 1600),
        'quality' => env('CMF_MEDIA_CROP_QUALITY', 0.92),
    ],
